    ...
    public class EfAuthorRepository : IAuthorRepository
    {
        ...
        public IReadOnlyList<Author> GetAll(string? nameContains = null)
        {
            if(string.IsNullOrEmpty(nameContains))
                ...
                return _context.Authors.Include(a => a.Books).Where(a => a.Name.Contains(nameContains)).ToList();
        }
        ...
        {
            return _context.Authors.Include(a => a.Books).Where(a =>a.Name == name).ToList();
        }

        public IReadOnlyList<Author> GetPaged(int skip, int take, string? nameContains = null)
        {
            if (string.IsNullOrEmpty(nameContains))
                return _context.Authors.Include(a => a.Books).OrderBy(a => a.Id).Skip(skip).Take(take).ToList();
            else
                return _context.Authors.Include(a => a.Books).Where(a => a.Name.Contains(nameContains)).OrderBy(a => a.Id).Skip(skip).Take(take).ToList();
        }
